...
object Parser extends Pipeline[Iterator[Token], Program]
  ...
  def opPos(string: String): Syntax[(String, Position, Position)] = accept(OperatorKind(string)) { 
    case op@OperatorToken(v) => (v, op.position, op.endPosition)
  }
  ...
  val identifierPos: Syntax[(String, Position, Position)] = accept(IdentifierKind) {
    case id@IdentifierToken(name) => (name, id.position, id.endPosition)
  }
  ...
  lazy val identifierType: Syntax[TypeTree] = (identifierPos ~ opt("." ~ identifier)).map {
    case (id, pos, endPos) ~ None => TypeTree(ClassType(QualifiedName(None, id))).setPos(pos).setEndPos(endPos)
    case (mod, pos, endPos) ~ Some(_ ~ id) => TypeTree(ClassType(QualifiedName(Some(mod), id))).setPos(pos).setEndPos(endPos)
  }
  ...
  lazy val unaryOp: Syntax[Expr] = (opt((opPos("!") | opPos("-"))) ~ simpleExpr).map {
    case Some(("!", pos, endPos)) ~ e => Not(e).setPos(pos).setEndPos(endPos)
    case Some(("-", pos, endPos)) ~ e => Neg(e).setPos(pos).setEndPos(endPos)
  ...
  lazy val caseClassPattern: Syntax[Pattern] = 
    (identifierPos ~ opt("." ~ identifier) ~ opt("(" ~ repsep(pattern, ",") ~ ")")).map{
      // case abc 
      case (id, pos, endPos) ~ None ~ None                            => IdPattern(id).setPos(pos).setEndPos(endPos)
      // case List(head,tail)
      case (id, pos, endPos) ~ None ~ Some(_ ~ patterns ~ _)          => CaseClassPattern(QualifiedName(None, id), patterns.toList).setPos(pos).setEndPos(endPos)
      // case Std.Lib
      case (mod, pos, endPos) ~ Some(_ ~ id) ~ None                   => CaseClassPattern(QualifiedName(Some(mod), id), Nil).setPos(pos).setEndPos(endPos)
      // case Std.List(head,tail)
      case (mod, pos, endPos) ~ Some(_ ~ id) ~ Some(_ ~ patterns ~ _) => CaseClassPattern(QualifiedName(Some(mod), id), patterns.toList).setPos(pos).setEndPos(endPos)
    }
  ...
  lazy val variableOrCall: Syntax[Expr] = (identifierPos ~ opt("." ~ identifier) ~ opt("(" ~ repsep(expr, ",") ~ ")")).map {
      // case abc 
      case (id, pos, endPos) ~ None ~ None                            => Variable(id).setPos(pos).setEndPos(endPos)
      // case add(a,b)
      case (id, pos, endPos) ~ None ~ Some(_ ~ params ~ _)            => Call(QualifiedName(None, id), params.toList).setPos(pos).setEndPos(endPos)
      // case add() 
      case (mod, pos, endPos) ~ Some(_ ~ id) ~ None                   => Call(QualifiedName(Some(mod), id), Nil).setPos(pos).setEndPos(endPos)
      // case Std.List(head,tail)
      case (mod, pos, endPos) ~ Some(_ ~ id) ~ Some(_ ~ params ~ _)   => Call(QualifiedName(Some(mod), id), params.toList).setPos(pos).setEndPos(endPos)
